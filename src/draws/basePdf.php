...
!   File       : $Source: /cvsroot/aotb/badnet/src/draws/basePdf.php,v $
!   Version    : $Name:  $
...
class drawsBasePdf extends utBase
{

  // {{{ getGroupPairs
  /**
   * Return the pairs of a group
   *
   * @access public
   */
  function getGroupPairs($roundId)
    {
      // for each pair of the group, get the players names
      $fields = array('pair_id', 'regi_longName', 
		      'asso_noc','team_noc', 'regi_noc',
		      'asso_logo', 'team_logo', 'team_stamp', 
		      't2r_tds', 'rkdf_label', 'i2p_cppp', 'i2p_classe', 'mber_licence');
      ...
      while ($entry = $res->fetchRow(DB_FETCHMODE_ASSOC))
	{
	  if ($pairId != $entry['pair_id'])
	    $row['tds'] =  $ut->getLabel($entry['t2r_tds']);
	  else
	    $row['tds'] =  $entry['t2r_tds'];
	  $pairId = $entry['pair_id'].

	  $row['pairId']= $entry['pair_id'];
	  $row['name']  = $entry['regi_longName'];
	  $row['club']  = $entry['team_stamp'];
	  $row['level'] = $entry['rkdf_label'];
	  $row['points']  = $entry['i2p_cppp'];
	  $row['rank']  = $entry['i2p_classe'];
	  $row['licence']  = $entry['mber_licence'];
	  if ($entry['regi_noc'] != '')
	    $row['noc']  = $entry['regi_noc'];
	  else if ($entry['team_noc'] != '')
	    $row['noc']  = $entry['team_noc'];
	  else 
	    $row['noc']  = $entry['asso_noc'];
	  if ($entry['team_logo'] != '')
	    $row['flag']  = utimg::getPathFlag($entry['team_logo']);
	  else if ($entry['asso_logo'] != '')
	    $row['flag']  = utimg::getPathFlag($entry['asso_logo']);
	  else
	    $row['flag']  = '';
	  $rows[] = $row;
	}
  ...
  // {{{ getGroupMatchs
  /**
   * Return the matches of a group
   *
   * @access public
   */
  function getGroupMatchs($roundId)
    {
	...
	$matchs[] = new objMatch($match['mtch_id']);
      return $matchs;
    }
  ...
  function getGroupRank($roundId)
	...
	" AND p2m_result != ".WBS_RES_NOPLAY;
      ...
      $fields = array('t2r_pairId', 't2r_rank');
	...
	" AND t2r_roundId = rund_id";
	  ...
	  if (isset($pairs[$pair['t2r_pairId']]))
	    {
	      $pair['matchs'] = $pairs[$pair['t2r_pairId']]['matchW'];
	      $pair['points'] = $pairs[$pair['t2r_pairId']]['point'];
	      $pair['games'] = $pairs[$pair['t2r_pairId']]['gameW'] -
		             $pairs[$pair['t2r_pairId']]['gameL'];
	      $pair['rallies'] = $pairs[$pair['t2r_pairId']]['pointW'] -
		             $pairs[$pair['t2r_pairId']]['pointL'];
	      $rows[$pair['t2r_pairId']] = $pair;
	    }
	}
      return $rows;
    }
  ...
  function getDrawPairs($drawId, $sort)
    {
      ...
      $fields = array('pair_id', 'pair_status', 'pair_order', 'pair_wo', 't2r_tds',);
      ...
      while($pair = $res->fetchRow(DB_FETCHMODE_ASSOC))
	{
      ...
      $fields = array('pair_id', 'regi_longName', 'rkdf_label',
		      'pair_intRank', 'pair_natRank', 'pair_status', 
		      'null as tds',
		      'pair_wo', 'asso_pseudo', 'regi_id', 'asso_id', 
  ...
  function getRoundPairs($roundId)
    {
      $ut = new utils();
      // Find the id of the pairs
      $fields = array('t2r_pairId', 't2r_tds', 't2r_status');
      $tables = array('t2r');
