...
'2' => OMEMBER_GENDER_FEMALE,
	...
	{
		$rows = array();
		$dbName = Oextbadminton::getDbName($aCriteria['season']);
		...
		if ( strlen($disci) && count($discis) && count($discis)< 5 )
		{
			...
			{
				$tmp[] = "EVN_TABLEAU_DISCIPLINE LIKE '%" . Oextbadminton::getDisciPoona($disci) . "%'";
			}
			$q->addWhere( '(' . implode(' OR ', $tmp) . ')');
		}
	...
	function searchInstance($aStr)
	{
		...
		$q->setFields('ins_id as id, ins_nom as name, ins_sigle as stamp, ins_ville as town');
	...
	function getInstance($aInstanceId)
	{
		...
		{
			$assoc['type'] = $this->_instanceType[$assoc['type']];
		}
		$assoc['id'] = -1;
		$assoc['noc'] = 'FRA';
		$assoc['ligue'] = '';
	 ...
	 */
	public function getPlayer($aLicense, $aSeason=null)
	{
		$dbName = Oextbadminton::getDbName($aSeason);
		$dbNameP = Bn::getConfigValue('base', 'database_poona');
		$q = new Bn_query("$dbNameP.PERSONNE", '_poona');
		$q->addField('PER_PES_ID + 5', 'gender');

		$q->addTable("$dbNameP.JOUEUR_CATEGORIE", 'JOC_ID=JOU_JOC_ID');
		$q->addField('JOC_NOM_LONG', 'labcatage');
		$q->addField('JOC_ID', 'catage');

		$q->addTable("$dbNameP.TYPE_CLASSEMENT s", 'JOU_TCL_ID_SIMPLE_OFF=s.TCL_ID');
		$q->addField('s.TCL_NOM', 'levels');
		$q->addTable("$dbNameP.TYPE_CLASSEMENT d", 'JOU_TCL_ID_DOUBLE_OFF=d.TCL_ID');
		$q->addField('d.TCL_NOM', 'leveld');
		$q->addTable("$dbNameP.TYPE_CLASSEMENT m", 'JOU_TCL_ID_MIXTE_OFF=m.TCL_ID');
		$q->addField('m.TCL_NOM', 'levelm');

		$q->addTable("$dbNameP.INSTANCE", 'JOU_INS_ID=INS_ID');
		$q->addField('INS_NOM', 'assoc');
		$q->addField('INS_ID', 'instanceid');
		$q->addField('INS_INS_ID_LIGUE', 'ligueid');
		$q->addField('INS_INS_ID_CODEP', 'codepid');

		$q->addTable("`$dbName`.JOUEUR", 'JOU_PER_ID=PER_ID');
		...
		$q->addWhere('PER_LICENCE=' . $aLicense);
		$player = $q->getRow();
		if ( ! $q->isError() && !empty($player))
		{
			...
			$ranks = $this->getRanks($player['gender'], $player['points'], $player['pointd'], $player['pointm']);
			$player['ranks'] = $ranks['ranks'];
			$player['rankd'] = $ranks['rankd'];
			$player['rankm'] = $ranks['rankm'];
	...
	public function getRanks($aGender, $aPoints, $aPointd, $aPointm)
	{
		...
		$dbName = Oextbadminton::getDbName();
	...
	function getPlayers($aCriteria)
	{
		...
		// Filtre sur les categories d'age
		$where = Oextbadminton::_wcatage($aCriteria->catage);
		...
		foreach($players as $player)
		{
			...
			$row['level'] = $player['levels'] .',' .$player['leveld'] .',' .$player['levelm'];
			$rows[] = $row;
		}
		return $rows;
	}
	...
	function searchPlayers($aCriteria)
	{
		...
		$q->addTable("$dbNameP.PERSONNE_SEXE", 'PER_PES_ID=PES_ID');
		$q->addField('PES_NOM_LONG', 'labgender');
		$q->addField('PES_ID + 5', 'gender');
		...
		if ( !empty($aCriteria['instanceId']) )
		{
			$q->addWhere('JOU_INS_ID =' . $aCriteria['instanceId']);
		}
		...
		*/
		...
		{
			$player['level'] = $player['levels'] .',' . $player['leveld'] . ',' . $player['levelm'];
			//$ranks = $this->getRanks($player['gender'], $player['ups'], $player['upd'], $player['upm']);
			//$player['rank']  = $ranks['ranks'] .',' . $ranks['rankd'] . ',' . $ranks['rankm'];
			$player['rank']  = '';
			$players[] = $player;
		}
